...
async function repeatedForwardA(map, start, goal) {
    ...
    trueMap[goal.x][goal.y] = 4;
    trueMap[start.x][start.y] = 5;
    //console.log(trueMap);
    console.log('broga')
    ...
    do {
        ...
        validPath = checkPath(path, map, trueMap, goal);
...
function checkPath(path, map, trueMap, goal) {
...
function repeatedForwardAHelper(trueMap, start, goal) {
    ...
    //displayA(trueMap);
    goal.g = 0;
    goal.h = caculateHeuristic(start, goal);
    goal.f = goal.g + goal.h;
    openList.enqueue(goal); //put start into open list;
